...
public class PortfolioService {
    ...
    /**
     * Elimina completamente una criptovaluta dal portafoglio (Database).
     * @param assetId L'ID univoco della crypto (es. "bitcoin", "ethereum")
     * @return true se l'eliminazione è avvenuta con successo, false in caso di errore
     */
    public boolean deleteAsset(String assetId) {
        try {
            assetDao.deleteAsset(assetId); // Chiama il metodo originale del tuo DAO
            return true;
        } catch (Exception e) {
            System.err.println("Errore durante l'eliminazione dell'asset nel PortfolioService: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
